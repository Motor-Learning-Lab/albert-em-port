...
def fit_one_state(
    ...
    for n in range(num_iterations):
        # E-step:
        # Get the smoothed Kalman estimates of states, variances, and
        # covariances using the one-state Kalman smoother
        xnN, VnN, Vnp1nN = kalman_smoother_one_state(parameters, y, e)

        # M-step:
        # Perform maximum likelihood estimation in a constrained parameter
        # space
        parameters = m_step_one_state(parameters, xnN, VnN, Vnp1nN, y, e, search_space)

        # Compute the incomplete log-likelihood for this parameter set
        likelihoods[n] = incomplete_log_likelihood_one_state(y, e, parameters)
